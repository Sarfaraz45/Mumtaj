...
public partial class HR_Default : System.Web.UI.Page
{
    ...
    public static string Approve(string id, string UserID, string cycleid)
    {
        ...
        SqlDataAdapter da = new SqlDataAdapter("select Top(1) * from TASK_CYCLE where IsAdministrator=0  and TaskOrder > '" + cycleid + "' Order by TaskOrder", Con);
    ...
    public static string LoadRegion()
    {
        ...
        RegionList.Clear();
            ...
            SqlDataAdapter da = new SqlDataAdapter("SELECT        dbo.TASK_MASTER.TASKID, dbo.TASK_MASTER.TASKTITLE, CONVERT(Nvarchar(50), dbo.TASK_MASTER.STARTDATE, 106) AS StartDate, CONVERT(Nvarchar(50), dbo.TASK_MASTER.ENDDATE, 106) AS EndDate,  STP_AREA_1.AreaTitle + ' ' + dbo.STP_AREA.AreaTitle AS Area, dbo.STP_ACTIVITY.ATTITLE, dbo.TASK_STATUS.TaskCycleID, dbo.TASK_STATUS.ApprovalDuration, dbo.TASK_STATUS.DesID, dbo.TASK_STATUS.STATUS,  dbo.TASK_STATUS.ISACTIVE FROM            dbo.TASK_MASTER INNER JOIN dbo.STP_AREA AS STP_AREA_1 ON dbo.TASK_MASTER.AREAID = STP_AREA_1.AreaID INNER JOIN dbo.STP_AREA ON STP_AREA_1.PID = dbo.STP_AREA.AreaID INNER JOIN dbo.STP_ACTIVITY ON dbo.TASK_MASTER.ACTID = dbo.STP_ACTIVITY.ACTID INNER JOIN dbo.TASK_STATUS ON dbo.TASK_MASTER.TASKID = dbo.TASK_STATUS.TASKID WHERE        (dbo.TASK_MASTER.ISDELETE = 0) AND (dbo.TASK_MASTER.ISCOMP = 0) AND (dbo.TASK_STATUS.ISACTIVE = 1)  ", Conn);
            DataTable dt = new DataTable();
            da.Fill(dt);
    ...
    public static string LoadRegionTask(string TaskID)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                string htm = "";
                SqlDataAdapter dda = new SqlDataAdapter("SELECT        TOP (100) PERCENT dbo.TASK_MASTER.TASKID, dbo.TASK_MASTER.TASKTITLE, dbo.TASK_MASTER.STARTDATE, dbo.TASK_MASTER.ENDDATE, dbo.TASK_MASTER.ISCOMP, dbo.TASK_MASTER.ISDELETE,  dbo.STP_Designation.DesID, dbo.STP_Designation.DesTitle, dbo.TASK_STATUS.ApprovalDuration, dbo.TASK_STATUS.STATUS, dbo.TASK_STATUS.ISACTIVE, dbo.TASK_STATUS.STATUSBY, dbo.TASK_STATUS.REMARKS,  dbo.TASK_STATUS.STATUSDATE, dbo.TASK_STATUS.TSID FROM            dbo.TASK_MASTER INNER JOIN dbo.TASK_STATUS ON dbo.TASK_MASTER.TASKID = dbo.TASK_STATUS.TASKID INNER JOIN dbo.STP_Designation ON dbo.TASK_STATUS.DesID = dbo.STP_Designation.DesID WHERE        (dbo.TASK_MASTER.ISCOMP = 0) AND (dbo.TASK_MASTER.ISDELETE = 0) and   dbo.TASK_MASTER.TASKID = '"+TaskID+"' ORDER BY dbo.TASK_STATUS.TSID", Conn);
                ...
                if (dtt.Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < dtt.Rows.Count; j++)
                    {
                        if (cc == 0)
                        {
                            htm = htm + dtt.Rows[j]["DesTitle"].ToString() + '^' + dtt.Rows[j]["STATUSDATE"].ToString();
                        }
                        else
                        {
                            htm = htm + "`" + dtt.Rows[j]["DesTitle"].ToString() + '^' + dtt.Rows[j]["STATUSDATE"].ToString();
