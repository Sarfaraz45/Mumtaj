...
function LoadSaveToastr() {
    Command: toastr["success"]("Vehicle has been saved Successfully.")
...
function LoadUpdateToastr() {
    Command: toastr["info"]("Vehicle has been updated Successfully.")
...
function LoadDeleteToastr() {
    Command: toastr["error"]("Vehicle has been deleted Successfully.")
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(jsdata, function (key, value) {
                ...
                document.getElementById("ddlHead").value = value.VTID;
                CheckDataLength();
...
function LoadRegion() {
    $.ajax({
        ...
        success: function (data) {
            ...
            ro = "<table id='dt-basic-example'class='table table-sm  dataTable dtr-inline table-hover' ><thead class='thead-themed'> <th>Title</th><th>Description</th><th>Reg #</th><th style='text-align:center;'>Action</th></thead><tbody>";
            $.each(jsdata, function (key, value) {

                ro += "<tr><td   class='two' style='width:30%;'>" + value.Name + "</td><td   class='three' style='width:30%;'>" + value.VDESC + "</td><td   class='three' style='width:20%;'>" + value.REGNO + "</td><td style='text-align:center;width:20%;'><button class='btn buttons-selected btn-primary btn-sm mr-1' tabindex='0' aria-controls='dt-basic-example' type='button'  onclick='LoadDetail(\"" + value.ID + "\");'><span><i class='fal fa-edit mr-1'></i> Select</span></button><button class='btn buttons-selected btn-danger btn-sm mr-1' tabindex='0' aria-controls='dt-basic-example' type='button'  onclick='DeleteData(\"" + value.ID + "\");'><span><i class='fal fa-times mr-1'></i> Delete</span></button></td></tr>";
...
function InsertData() {
    ...
    var ddlHead = document.getElementById('ddlHead').value;
        ...
        $.ajax({
            type: 'POST',
            url: 'Vehicle.aspx/SaveTransaction',
            //data: {},
...
function UpdateData() {
    ...
    $.ajax({
        type: 'POST',
        url: 'Vehicle.aspx/UpdateTransaction',
        //data: {},
...
function DeleteData(id) {
    ...
    $.ajax({
        type: 'POST',
        url: 'Vehicle.aspx/DeleteTransaction',
        //data: {},
