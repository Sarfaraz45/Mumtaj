...
public partial class HR_Default : System.Web.UI.Page
{
    ...
    public static string LoadRegion()
    {
        ...
        SqlDataAdapter da = new SqlDataAdapter("SELECT     * from PRL_Allowance where IsDelete=0 order by 1", Conn);
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                dbdc.ID = dt.Rows[i]["AllowanceID"].ToString();
                dbdc.Name = dt.Rows[i]["AllowanceTitle"].ToString();
                dbdc.VDESC = dt.Rows[i]["Amount"].ToString(); 
    ...
    public static string LoadDetail(string id)
    {
        ...
        SqlDataAdapter da = new SqlDataAdapter("SELECT     * from PRL_Allowance where IsDelete=0 and AllowanceID='" + id + "' order by 1", Conn);
        DataTable dt = new DataTable();
        da.Fill(dt);
    ...
    public static string SaveTransaction(string Title, string txtDescription, string UserID)
    {
        ...
        SqlCommand cmd = new SqlCommand("insert into PRL_Allowance (AllowanceTitle,Amount,CREATEBY) values ( '" + Title + "','" + txtDescription + "','" + UserID + "')", Con);
        Con.Open();
        cmd.ExecuteNonQuery();
        Con.Close();
    ...
    public static string UpdateTransaction(string Title, string txtDescription, string id, string UserID)
    {
        ...
        SqlCommand cmd = new SqlCommand("update PRL_Allowance  set  AllowanceTitle='" + Title + "',Amount='" + txtDescription + "' where AllowanceID='" + id + "'", Con);
        Con.Open();
        cmd.ExecuteNonQuery();
        Con.Close();
    ...
    public static string DeleteTransaction(string id, string UserID)
    {
        ...
        SqlCommand cmd = new SqlCommand("update PRL_Allowance  set IsDelete=1,DELETEBY='" + UserID + "',DELETEDATE='" + DateTime.Now + "' where AllowanceID='" + id + "'", Con);
        Con.Open();
        cmd.ExecuteNonQuery();
        Con.Close();
