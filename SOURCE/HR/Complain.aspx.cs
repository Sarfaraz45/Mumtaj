...
/////////////            InnerHTML = InnerHTML + " </div>";
    ...
    public static string LoadHead()
    {
        ...
        string str = "select * from STP_ACTIVITY where IsDelete=0 order by ATTITLE";
    ...
    public static string LoadShift()
    {
        ...
        //string str = "select * from STP_Employee where IsDelete=0 order by FirstName";
        string str = "select * from Shift where IsDelete=0 order by ShiftTitle";
    ...
    public static string SaveTransaction(string Title, string UID, string dta, string Start,  string txtRemarks, string AreaID, string ddlShift, string dtaVehicle,string UserID)
    {
        ...
        string vNo = "";
        SqlConnection Con = new SqlConnection(ConfigurationManager.ConnectionStrings["Con"].ConnectionString);
        string a = AACommon.GetAlphaNumericIDSIX("TASK_MASTER", "COM-", "TASKID", Con);
        SqlCommand cmd = new SqlCommand("insert into TASK_MASTER (TASKID,TASKTITLE,STARTDATE,AREAID,ACTID,ShiftID,CREATEBY,ISCOMP) values ('" + a + "','" + Title + "','" + Start + "','" + AreaID + "','" + UID + "','" + ddlShift + "','" + UserID + "','1')", Con);
        Con.Open();
        cmd.ExecuteNonQuery();
        Con.Close();
