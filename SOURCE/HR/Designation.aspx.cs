...
public partial class HR_Default : System.Web.UI.Page
{
    ...
    public static string LoadRegion()
    {
        ...
        SqlDataAdapter da = new SqlDataAdapter("SELECT     * from STP_Designation where IsDelete=0 order by 1", Conn);
        ...
        if (dt.Rows.Count > 0)
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                dbdc.VDESC = dt.Rows[i]["JobDesc"].ToString(); 
    ...
    public static string LoadDetail(string id)
    {
        ...
        SqlDataAdapter da = new SqlDataAdapter("SELECT     * from STP_Designation where IsDelete=0 and DesID='" + id + "' order by 1", Conn);
        DataTable dt = new DataTable();
        da.Fill(dt);
    ...
    public static string SaveTransaction(string Title, string txtDescription, string UserID)
    {
        ...
        string a = AACommon.GetAlphaNumericIDSIX("STP_Designation", "DES-", "DesID", Con);
        SqlCommand cmd = new SqlCommand("insert into STP_Designation (DesID,DesTitle,JobDesc,CREATEBY) values ( '" + a + "','" + Title + "','" + txtDescription + "','" + UserID + "')", Con);
        Con.Open();
        cmd.ExecuteNonQuery();
        Con.Close();
    ...
    public static string UpdateTransaction(string Title, string txtDescription, string id, string UserID)
    {
        ...
        SqlCommand cmd = new SqlCommand("update STP_Designation  set  DesTitle='" + Title + "',JobDesc='" + txtDescription + "' where DesID='" + id + "'", Con);
        Con.Open();
        cmd.ExecuteNonQuery();
        Con.Close();
    ...
    public static string DeleteTransaction(string id, string UserID)
    {
        ...
        SqlCommand cmd = new SqlCommand("update STP_Designation  set IsDelete=1,DELETEBY='" + UserID + "',DELETEDATE='" + DateTime.Now + "' where DesID='" + id + "'", Con);
        Con.Open();
        cmd.ExecuteNonQuery();
        Con.Close();
