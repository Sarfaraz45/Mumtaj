...
public partial class HR_Default : System.Web.UI.Page
{
    ...
    public static string LoadRegion(string DesID)
    {
       ...
       // string DesID = "DES-000030"; //Director
      
        SqlDataAdapter dacycle = new SqlDataAdapter("select  Top(1) * from TASK_CYCLE where  DesID='" + DesID + "' and IsAdministrator=0 and TaskOrder <> 1", Conn);
    ...
    public static string LoadVehicleDetail(string TaskID)
    {
        ...
        string MasterHTML = "";
        MasterHTML = "<table  class='table table-sm  dataTable dtr-inline'  id='tablepagingVehDetail' ><thead class='bg-primary-500'> <th>Vehicle</th><th>Driver</th></thead><tbody>";
        SqlConnection Conn = new SqlConnection(ConfigurationManager.ConnectionStrings["Con"].ConnectionString);
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            if (rowGroup != currentGroup)
            {
                ...
                MasterHTML = MasterHTML + "<tr><td colspan=2  style='font-weight: bolder;background: #c5c5bf'> " + dt.Rows[i]["VTTITLE"].ToString() + "</td> </tr>";
                NewVt = OldVt;
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (dtt.Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < dtt.Rows.Count; j++)
                    {
                        if (cc == 0)
                        {
                            htm = htm + dtt.Rows[j]["VTTITLE"].ToString() + '^' + dtt.Rows[j]["QTY"].ToString();
                        }
                        else
                        {
                            htm = htm + "`"+dtt.Rows[j]["VTTITLE"].ToString() + '^' + dtt.Rows[j]["QTY"].ToString();
