...
public partial class PROCUREMENT_RequistionLIST : System.Web.UI.Page
{
    ...
    public static string LoadLIST(string UserID)
    {
        ...
        htm = htm + "<th>Requisition At</th>";
        htm = htm + "<th></th>";
        htm = htm + "</tr>";
        htm = htm + "</thead>";
        htm = htm + "<tbody>";
        SqlConnection Con = new SqlConnection(ConfigurationManager.ConnectionStrings["Con"].ConnectionString);
        DataSet ds = AACommon.ReturnDatasetBySP("PROC_REQUISITION_INPROCESS_LIST", Con, null);
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            htm = htm + "<td>" + dt.Rows[i]["ReqAtName"].ToString() + "</td>";
            htm = htm + "<td><a href='../REPORTS/Req_His.aspx?ID=" + dt.Rows[i]["ReqID"].ToString() + "' target='_blank'>View History</a></td>";
            htm = htm + "</tr>";
        }
        htm = htm + "</tbody>";
        htm = htm + "</table>";
